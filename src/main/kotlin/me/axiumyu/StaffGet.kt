...
class StaffGet : CommandExecutor {
    override fun onCommand(
        ...
        if (xc.getPlayerData(p0.name).balance < BigDecimal(500)) {
            p0.sendMessage(text("余额不足，需要${xc.getdisplay(BigDecimal(400))}").color(color(0xffea3a)))
            return false
        }
        xc.changePlayerBalance(p0.uniqueId, p0.name, BigDecimal(400), false)
